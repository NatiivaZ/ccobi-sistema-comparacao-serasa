...
if 'resultados' in st.session_state:
    ...
    with tab1:
        ...
        if not resultados['df_serasa_filtrado'].empty and not resultados['df_divida_filtrado'].empty:
            ...
            if tem_protocolo:
                st.info("💡 Exporte os autos que estão em **AMBAS as bases** (SERASA e Dívida Ativa). **IMPORTANTE:** Antes da exportação principal, o sistema **exclui autuados classificados como Órgão, Banco ou Leasing** e também **remove autos decadentes linha por linha**. Cada exportação tem um botão separado para baixar somente os autos decadentes. A classificação por faixa de valor é feita pela **SOMA dos valores não decadentes por CPF/CNPJ**. Cada arquivo contém: Auto de Infração, **Número de Protocolo**, CPF/CNPJ e **Valor Individual de cada auto**, ordenados por CPF/CNPJ.")
            else:
                st.info("💡 Exporte os autos que estão em **AMBAS as bases** (SERASA e Dívida Ativa). **IMPORTANTE:** Antes da exportação principal, o sistema **exclui autuados classificados como Órgão, Banco ou Leasing** e também **remove autos decadentes linha por linha**. Cada exportação tem um botão separado para baixar somente os autos decadentes. A classificação por faixa de valor é feita pela **SOMA dos valores não decadentes por CPF/CNPJ**. Cada arquivo contém: Auto de Infração, CPF/CNPJ e **Valor Individual de cada auto**, ordenados por CPF/CNPJ.")
            
            # A base de exportação parte do resultado já tratado pela análise principal.
            def mascara_documento_aproveitavel(serie):
                """Marca documentos que continuam úteis depois da normalização."""
                if serie is None:
                    return pd.Series(dtype=bool)
                doc_norm = serie.apply(normalizar_cpf_cnpj)
                return doc_norm.notna() & doc_norm.astype(str).str.contains(r"\d", regex=True)

            def construir_mapa_cpf_por_auto(df_base):
                """Monta um mapa AUTO_NORM -> CPF/CNPJ usando apenas documentos aproveitáveis."""
                if (
                    df_base is None
                    or df_base.empty
                    or 'AUTO_NORM' not in df_base.columns
                    or coluna_cpf_cnpj not in df_base.columns
                ):
                    return {}

                docs_validos = mascara_documento_aproveitavel(df_base[coluna_cpf_cnpj])
                df_docs = df_base.loc[
                    df_base['AUTO_NORM'].notna() & docs_validos,
                    ['AUTO_NORM', coluna_cpf_cnpj]
                ].copy()
                if df_docs.empty:
                    return {}
                return df_docs.drop_duplicates(subset=['AUTO_NORM'], keep='first').set_index('AUTO_NORM')[coluna_cpf_cnpj].to_dict()

            mapa_cpf_divida_filtrado = construir_mapa_cpf_por_auto(resultados.get('df_divida_filtrado'))
            mapa_cpf_divida_original = construir_mapa_cpf_por_auto(resultados.get('df_divida_original'))
            mapa_cpf_serasa_filtrado = construir_mapa_cpf_por_auto(resultados.get('df_serasa_filtrado'))
            mapa_cpf_serasa_original = construir_mapa_cpf_por_auto(resultados.get('df_serasa_original'))

            def normalizar_nome_coluna_busca(valor):
                """Normaliza cabeçalhos para localizar variações de nome do autuado."""
                texto = unicodedata.normalize("NFD", str(valor).strip().upper())
                texto = "".join(c for c in texto if unicodedata.category(c) != "Mn")
                texto = "".join(c if c.isalnum() else " " for c in texto)
                return " ".join(texto.split())

            def resolver_coluna_nome_autuado(df_base):
                """Localiza a coluna de nome do autuado mesmo com cabeçalhos diferentes."""
                ...
                mapa_colunas = {
                    normalizar_nome_coluna_busca(col): col
                    for col in df_base.columns
                }

                for candidato in candidatos:
                    col = mapa_colunas.get(normalizar_nome_coluna_busca(candidato))
                    if col:
                        return col

                for col_norm, col_original in mapa_colunas.items():
                    if "AUTUADO" in col_norm or "RAZAO SOCIAL" in col_norm:
                        return col_original

                return None

            def construir_mapa_nome_por_auto(df_base):
                """Monta AUTO_NORM -> Nome Autuado usando a melhor coluna encontrada."""
                col_nome = resolver_coluna_nome_autuado(df_base)
                if (
                    df_base is None
                    or df_base.empty
                    or 'AUTO_NORM' not in df_base.columns
                ...
                nomes_validos = df_base[col_nome].notna() & df_base[col_nome].astype(str).str.strip().ne("")
                df_nomes = df_base.loc[
                    df_base['AUTO_NORM'].notna() & nomes_validos,
                    ['AUTO_NORM', col_nome]
                ].copy()
                if df_nomes.empty:
                    return {}
                return df_nomes.drop_duplicates(subset=['AUTO_NORM'], keep='first').set_index('AUTO_NORM')[col_nome].to_dict()

            mapa_nome_divida_filtrado = construir_mapa_nome_por_auto(resultados.get('df_divida_filtrado'))
            mapa_nome_divida_original = construir_mapa_nome_por_auto(resultados.get('df_divida_original'))
            mapa_nome_serasa_filtrado = construir_mapa_nome_por_auto(resultados.get('df_serasa_filtrado'))
            mapa_nome_serasa_original = construir_mapa_nome_por_auto(resultados.get('df_serasa_original'))

            def resolver_cpf_cnpj_por_auto(df_work, df_export_local):
                """Busca o CPF/CNPJ do auto em todas as bases, priorizando a Dívida Ativa."""
                resultado = pd.Series([None] * len(df_work), index=df_work.index, dtype=object)

                if 'AUTO_NORM' in df_work.columns:
                    auto_norm = df_work['AUTO_NORM']
                elif coluna_auto in df_export_local.columns:
                    auto_norm = df_export_local[coluna_auto].apply(normalizar_auto)
                else:
                    auto_norm = pd.Series([None] * len(df_work), index=df_work.index)

                def preencher(candidato):
                    nonlocal resultado
                    if candidato is None:
                        return
                    candidato = pd.Series(candidato, index=df_work.index)
                    mask_candidato = mascara_documento_aproveitavel(candidato)
                    mask_resultado = mascara_documento_aproveitavel(resultado)
                    mask_preencher = mask_candidato & ~mask_resultado
                    if mask_preencher.any():
                        resultado.loc[mask_preencher] = candidato.loc[mask_preencher]

                # 1) Linha mesclada da Dívida; 2) mapas por auto da Dívida; 3) SERASA.
                preencher(df_work.get(f"{coluna_cpf_cnpj}_divida"))
                preencher(auto_norm.map(mapa_cpf_divida_filtrado))
                preencher(auto_norm.map(mapa_cpf_divida_original))
                preencher(df_work.get(f"{coluna_cpf_cnpj}_serasa"))
                preencher(auto_norm.map(mapa_cpf_serasa_filtrado))
                preencher(auto_norm.map(mapa_cpf_serasa_original))
                preencher(df_work.get(coluna_cpf_cnpj))
                preencher(df_export_local.get(coluna_cpf_cnpj))

                return resultado.fillna('NÃO INFORMADO')

            def resolver_nome_autuado_por_auto(df_work, df_export_local):
                """Busca o nome do autuado pelo auto em todas as bases disponíveis."""
                resultado = pd.Series([""] * len(df_work), index=df_work.index, dtype=object)
                ...
                def preencher(candidato):
                    ...
                    if candidato is None:
                        return
                    candidato = pd.Series(candidato, index=df_work.index).fillna("").astype(str).str.strip()
                    mask_preencher = candidato.ne("") & resultado.astype(str).str.strip().eq("")
                    if mask_preencher.any():
                        resultado.loc[mask_preencher] = candidato.loc[mask_preencher]

                col_nome_work = resolver_coluna_nome_autuado(df_work)
                col_nome_export = resolver_coluna_nome_autuado(df_export_local)
                preencher(df_work[col_nome_work] if col_nome_work else None)
                preencher(df_export_local[col_nome_export] if col_nome_export else None)
                preencher(auto_norm.map(mapa_nome_serasa_filtrado))
                preencher(auto_norm.map(mapa_nome_serasa_original))
                preencher(auto_norm.map(mapa_nome_divida_filtrado))
                preencher(auto_norm.map(mapa_nome_divida_original))

                return resultado

            if 'df_final_sql' in resultados and not resultados['df_final_sql'].empty:
                    cache_df_export_key = f"cache_df_export_cpf_auto_nome_v4::{st.session_state.get('_analise_hash', 'default')}"
                    ...
                    def montar_df_export():
                        ...
                        if coluna_auto in df_export_local.columns and len(df_export_local) == len(df_final_work):
                            ...
                            df_export_local[coluna_cpf_cnpj] = resolver_cpf_cnpj_por_auto(df_final_work, df_export_local)
                            ...
                            df_export_local['Nome Autuado'] = resolver_nome_autuado_por_auto(df_final_work, df_export_local)
            ...
            if df_export.empty:
                ...
                def garantir_coluna_situacao_decadente(df_base):
                    """Garante a coluna de decadência para separar os autos por linha."""
                    if df_base is None or df_base.empty:
                        return df_base
                    df = df_base.copy()
                    if 'Situação decadente' not in df.columns:
                        df['Situação decadente'] = calcular_situacao_decadente(df, coluna_modal=coluna_modal_serasa)
                    return df

                def separar_por_situacao_decadente(df_base):
                    """Separa autos decadentes e não decadentes sem descartar o CPF/CNPJ inteiro."""
                    df = garantir_coluna_situacao_decadente(df_base)
                    if df is None or df.empty:
                        return df, df
                    situacao = df['Situação decadente'].fillna('').astype(str).str.strip()
                    return df[situacao == ''].copy(), df[situacao != ''].copy()

                def preparar_dados_exportacao(df_base, filtro_valor=None, somente_decadentes=False):
                    ...
                    # A regra agora é global: decadentes saem antes da soma por CPF/CNPJ.
                    df_nao_decadente, df_decadente = separar_por_situacao_decadente(df_base)

                    def preparar_base_para_faixa(df_origem):
                        """Normaliza valor e deduplica a base antes da soma/exportação."""
                        if df_origem is None or df_origem.empty:
                            return pd.DataFrame()
                        df_work = df_origem.copy()

                        # Garante que o valor esteja numérico antes de filtrar.
                        if coluna_valor in df_work.columns:
                            if df_work[coluna_valor].dtype not in ['int64', 'float64']:
                                df_work[coluna_valor] = pd.to_numeric(df_work[coluna_valor], errors='coerce')

                        # Valor vazio ou zero não entra na exportação.
                        if coluna_valor in df_work.columns:
                            df_work = df_work[
                                (df_work[coluna_valor].notna())
                                & (df_work[coluna_valor] > 0)
                            ].copy()
                        ...
                        return deduplicar_por_protocolo_ou_auto(
                            df_work,
                            coluna_auto,
                    ...
                    df_base_preparada = preparar_base_para_faixa(df_nao_decadente)
                    df_decadente_preparada = preparar_base_para_faixa(df_decadente)
                    cpf_filtro_faixa = None

                    # O filtro por faixa é feito em cima da soma por CPF/CNPJ.
                    if filtro_valor is not None and not df_base_preparada.empty and coluna_valor in df_base_preparada.columns:
                        ...
                        if 'CPF_CNPJ_NORM' in df_base_preparada.columns:
                            ...
                            if not df_base_para_agrupar.empty:
                                ...
                                if filtro_valor == 'ate_999':
                                    ...
                                    df_filtrado = df_base_preparada[df_base_preparada['CPF_CNPJ_NORM'].isin(cpf_filtro_faixa)].copy()
                    ...
                    if somente_decadentes:
                        if df_decadente_preparada.empty:
                            return None
                        if filtro_valor is not None:
                            if 'CPF_CNPJ_NORM' not in df_decadente_preparada.columns and coluna_cpf_cnpj in df_decadente_preparada.columns:
                                df_decadente_preparada['CPF_CNPJ_NORM'] = df_decadente_preparada[coluna_cpf_cnpj].apply(normalizar_cpf_cnpj)
                            if cpf_filtro_faixa is not None and 'CPF_CNPJ_NORM' in df_decadente_preparada.columns:
                                df_filtrado = df_decadente_preparada[df_decadente_preparada['CPF_CNPJ_NORM'].isin(cpf_filtro_faixa)].copy()
                            else:
                                df_filtrado = pd.DataFrame()
                        else:
                            df_filtrado = df_decadente_preparada.copy()
                    ...
                    col_nome_autuado_encontrada = resolver_coluna_nome_autuado(df_filtrado)

                    if col_nome_autuado_encontrada:
                        colunas_export['Nome Autuado'] = df_filtrado[col_nome_autuado_encontrada].fillna('').astype(str).str.strip()
                ...
                export_run_id = f"{st.session_state.get('export_run_id', 'default')}::decadentes_v1::cpf_auto_nome_v4"
                ...
                def render_exportacao_excel(
                    ...
                    empty_warning,
                    success_template,
                    extra_caption_fn=None,
                    producer_decadentes=None,
                    titulo_gerar_decadentes=None,
                    label_download_decadentes=None,
                    nome_arquivo_decadentes=None,
                    empty_warning_decadentes=None,
                ):
                    ...
                    if producer_decadentes:
                        titulo_base = titulo_gerar.replace("Gerar arquivo ", "").replace("Gerar planilha ", "")
                        nome_decadentes = nome_arquivo_decadentes or nome_arquivo.replace(".xlsx", " Decadentes.xlsx")
                        render_exportacao_excel(
                            chave=f"{chave}_decadentes",
                            titulo_gerar=titulo_gerar_decadentes or f"Gerar somente decadentes - {titulo_base}",
                            label_download=label_download_decadentes or f"📥 Download somente decadentes - {titulo_base}",
                            nome_aba="Decadentes",
                            nome_arquivo=nome_decadentes,
                            help_download="Arquivo Excel contendo apenas os autos com Situação decadente preenchida.",
                            producer=producer_decadentes,
                            empty_warning=empty_warning_decadentes or "⚠️ Nenhum auto decadente encontrado para esta exportação",
                            success_template="✅ {qtd:,} autos decadentes"
                        )
                
                def preparar_dados_apenas_serasa(df_apenas_serasa, filtro_valor=None, somente_decadentes=False):
                    ...
                    df = filtrar_autuados_cobraveis(df, coluna_nome_autuado)
                    if df is None or df.empty:
                        return None
                    ...
                    df['Situação decadente'] = calcular_situacao_decadente(df, coluna_modal=coluna_modal_serasa)
                    situacao_decadente = df['Situação decadente'].fillna('').astype(str).str.strip()
                    df_nao_decadente = df[situacao_decadente == ''].copy()
                    df_decadente = df[situacao_decadente != ''].copy()

                    cpf_filtro_faixa = None
                    df_base_faixa = df_nao_decadente.copy()
                    if filtro_valor is not None:
                        if 'CPF_CNPJ_NORM' not in df_base_faixa.columns and coluna_cpf_cnpj in df_base_faixa.columns:
                            df_base_faixa['CPF_CNPJ_NORM'] = df_base_faixa[coluna_cpf_cnpj].apply(normalizar_cpf_cnpj)

                        if (
                            not df_base_faixa.empty
                            and 'CPF_CNPJ_NORM' in df_base_faixa.columns
                            and '_VALOR_NUM' in df_base_faixa.columns
                        ):
                            df_para_agrupar = df_base_faixa[df_base_faixa['CPF_CNPJ_NORM'].notna()].copy()
                            if not df_para_agrupar.empty:
                                agrupado = df_para_agrupar.groupby('CPF_CNPJ_NORM')['_VALOR_NUM'].sum().reset_index()
                                agrupado.columns = ['CPF_CNPJ_NORM', 'VALOR_TOTAL']
                                if filtro_valor == 'ate_999':
                                    cpf_filtro_faixa = set(agrupado[agrupado['VALOR_TOTAL'] < 1000]['CPF_CNPJ_NORM'].unique())
                                elif filtro_valor == 'acima_1000':
                                    cpf_filtro_faixa = set(agrupado[agrupado['VALOR_TOTAL'] >= 1000]['CPF_CNPJ_NORM'].unique())
                                elif filtro_valor == 'entre_500_999':
                                    cpf_filtro_faixa = set(
                                        agrupado[
                                            (agrupado['VALOR_TOTAL'] >= 500)
                                            & (agrupado['VALOR_TOTAL'] <= 999.99)
                                        ]['CPF_CNPJ_NORM'].unique()
                                    )
                                else:
                                    cpf_filtro_faixa = set(agrupado['CPF_CNPJ_NORM'].unique())

                            if cpf_filtro_faixa is not None:
                                df_base_faixa = df_base_faixa[df_base_faixa['CPF_CNPJ_NORM'].isin(cpf_filtro_faixa)].copy()
                            else:
                                df_base_faixa = pd.DataFrame()
                        else:
                            df_base_faixa = pd.DataFrame()
                    ...
                    if somente_decadentes:
                        if filtro_valor is not None:
                            if (
                                cpf_filtro_faixa is not None
                                and 'CPF_CNPJ_NORM' in df_decadente.columns
                            ):
                                df = df_decadente[df_decadente['CPF_CNPJ_NORM'].isin(cpf_filtro_faixa)].copy()
                            else:
                                df = pd.DataFrame()
                        else:
                            df = df_decadente.copy()
                    else:
                        df = df_base_faixa.copy()
                    ...
                    col_nome_serasa_export = resolver_coluna_nome_autuado(df)
                    if col_nome_serasa_export:
                        colunas_export['Nome Autuado'] = df[col_nome_serasa_export].fillna('').astype(str).str.strip()
                    ...
                    colunas_export['Valor'] = df['_VALOR_NUM']
                    ...
                    if 'Modais' in colunas_export:
                        ordem.append('Modais')
                    if 'Nome Autuado' in colunas_export:
                ...
                def preparar_dados_serasa_classificacao(df_serasa, somente_decadentes=False):
                ...
                render_exportacao_excel(
                    ...
                    success_template="✅ {qtd:,} autos não decadentes (todos)",
                    producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor=None, somente_decadentes=True),
                    titulo_gerar_decadentes="Gerar arquivo Decadentes - Todos os Autos",
                    label_download_decadentes="📥 Download Decadentes - Todos os Autos",
                    nome_arquivo_decadentes=f"Base Comparada Todos Decadentes {data_arquivo}.xlsx"
                )
                ...
                with col_exp1:
                    ...
                    render_exportacao_excel(
                        ...
                        success_template="✅ {qtd:,} autos não decadentes até R$ 999,99",
                        producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor='ate_999', somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes ≤ R$ 999,99",
                        label_download_decadentes="📥 Download Decadentes ≤ R$ 999,99",
                        nome_arquivo_decadentes=f"Base Comparada Ate 999 Decadentes {data_arquivo}.xlsx"
                    )
                ...
                with col_exp2:
                    ...
                    render_exportacao_excel(
                        ...
                        success_template="✅ {qtd:,} autos não decadentes na faixa R$ 500–R$ 999,99",
                        producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor='entre_500_999', somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes R$ 500–R$ 999,99",
                        label_download_decadentes="📥 Download Decadentes R$ 500–R$ 999,99",
                        nome_arquivo_decadentes=f"Base Comparada 500 a 999 Decadentes {data_arquivo}.xlsx"
                    )
                ...
                with col_exp3:
                    ...
                    render_exportacao_excel(
                        ...
                        success_template="✅ {qtd:,} autos não decadentes acima de R$ 1.000,00",
                        producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor='acima_1000', somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes > R$ 1.000,00",
                        label_download_decadentes="📥 Download Decadentes > R$ 1.000,00",
                        nome_arquivo_decadentes=f"Base Comparada Acima 1000 Decadentes {data_arquivo}.xlsx"
                    )
                
                # A mesma lógica, mas sem cortar por vencimento.
                st.markdown("---")
                st.markdown("#### 📥 Exportar Base Sem Filtro de Data")
                st.info("💡 Exporte TODOS os autos que estão em ambas as bases, **independente da data de vencimento**. Os arquivos principais mantêm somente autos não decadentes; os autos decadentes ficam nos botões separados de decadentes.")
                
                if 'df_final_geral' in resultados and not resultados['df_final_geral'].empty:
                    cache_df_export_geral_key = f"cache_df_export_geral_cpf_auto_nome_v4::{st.session_state.get('_analise_hash', 'default')}"
                    ...
                    def montar_df_export_geral():
                        ...
                        if coluna_auto in df_export_geral_local.columns and len(df_export_geral_local) == len(df_final_geral_work):
                            ...
                            df_export_geral_local[coluna_cpf_cnpj] = resolver_cpf_cnpj_por_auto(df_final_geral_work, df_export_geral_local)
                            ...
                            df_export_geral_local['Nome Autuado'] = resolver_nome_autuado_por_auto(df_final_geral_work, df_export_geral_local)
                    ...
                    render_exportacao_excel(
                        ...
                        success_template="✅ {qtd:,} autos não decadentes (sem filtro de data)",
                        extra_caption_fn=lambda dados_df: "📋 Todos os autos não decadentes (sem filtro de data)",
                        producer_decadentes=lambda: preparar_dados_exportacao(df_export_geral, filtro_valor=None, somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes - Todos os Autos (Sem Filtro de Data)",
                        label_download_decadentes="📥 Download Decadentes - Todos os Autos (Sem Filtro de Data)",
                        nome_arquivo_decadentes=f"Base Comparada Sem Filtro Data Decadentes {data_arquivo}.xlsx"
                    )
                    ...
                    with col_geral_exp1:
                        ...
                        render_exportacao_excel(
                            ...
                            success_template="✅ {qtd:,} autos não decadentes até R$ 999,99 (sem filtro de data)",
                            producer_decadentes=lambda: preparar_dados_exportacao(df_export_geral, filtro_valor='ate_999', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes ≤ R$ 999,99 (Sem Filtro de Data)",
                            label_download_decadentes="📥 Download Decadentes ≤ R$ 999,99 (Sem Filtro de Data)",
                            nome_arquivo_decadentes=f"Base Comparada Ate 999 Sem Filtro Data Decadentes {data_arquivo}.xlsx"
                        )
                    ...
                    with col_geral_exp2:
                        ...
                        render_exportacao_excel(
                            ...
                            success_template="✅ {qtd:,} autos não decadentes na faixa R$ 500–R$ 999,99 (sem filtro de data)",
                            producer_decadentes=lambda: preparar_dados_exportacao(df_export_geral, filtro_valor='entre_500_999', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes R$ 500–R$ 999,99 (Sem Filtro de Data)",
                            label_download_decadentes="📥 Download Decadentes R$ 500–R$ 999,99 (Sem Filtro de Data)",
                            nome_arquivo_decadentes=f"Base Comparada 500 a 999 Sem Filtro Data Decadentes {data_arquivo}.xlsx"
                        )
                    ...
                    with col_geral_exp3:
                        ...
                        render_exportacao_excel(
                            ...
                            success_template="✅ {qtd:,} autos não decadentes acima de R$ 1.000,00 (sem filtro de data)",
                            producer_decadentes=lambda: preparar_dados_exportacao(df_export_geral, filtro_valor='acima_1000', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes > R$ 1.000,00 (Sem Filtro de Data)",
                            label_download_decadentes="📥 Download Decadentes > R$ 1.000,00 (Sem Filtro de Data)",
                            nome_arquivo_decadentes=f"Base Comparada Acima 1000 Sem Filtro Data Decadentes {data_arquivo}.xlsx"
                        )
                
                # Exportação da base SERASA completa por faixas de valor.
                st.markdown("---")
                st.markdown("#### 📥 Exportar Base SERASA")
                st.info("💡 Esta exportação usa a **base inteira da SERASA**, sem excluir autos que também constam na Dívida Ativa. O arquivo principal traz somente autos não decadentes; o botão de decadentes baixa os autos removidos por Situação decadente.")
                st.caption("📐 **Como é feita a conta:** A base SERASA é deduplicada por Auto de Infração antes dos filtros. As faixas de valor usam a **soma dos autos não decadentes por CPF/CNPJ**.")
                if 'df_serasa_original' in resultados and resultados['df_serasa_original'] is not None and not resultados['df_serasa_original'].empty:
                    df_base_serasa_export = resultados['df_serasa_original']
                    st.markdown("##### 📥 Todos os Autos da Base SERASA")
                    st.markdown("**Sem filtro de valor**")
                    render_exportacao_excel(
                        chave="base_serasa_todos",
                        titulo_gerar="Gerar arquivo Base SERASA",
                        label_download="📥 Download Base SERASA",
                        nome_aba="Base_SERASA",
                        nome_arquivo=f"Base SERASA {data_arquivo}.xlsx",
                        help_download="Arquivo Excel com a base SERASA completa, deduplicada por auto, com coluna Data Infração.",
                        producer=lambda: preparar_dados_apenas_serasa(df_base_serasa_export),
                        empty_warning="⚠️ Nenhum dado disponível para exportar na base SERASA.",
                        success_template="✅ {qtd:,} autos não decadentes na base SERASA",
                        producer_decadentes=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes - Base SERASA",
                        label_download_decadentes="📥 Download Decadentes - Base SERASA",
                        nome_arquivo_decadentes=f"Base SERASA Decadentes {data_arquivo}.xlsx"
                    )

                    col_apenas_1, col_apenas_2, col_apenas_3 = st.columns(3)

                    with col_apenas_1:
                        st.markdown("##### 📥 Autos até R$ 999,99")
                        render_exportacao_excel(
                            chave="base_serasa_ate_999",
                            titulo_gerar="Gerar arquivo Base SERASA ≤ R$ 999,99",
                            label_download="📥 Download Base SERASA ≤ R$ 999,99",
                            nome_aba="Base_SERASA_Ate_999",
                            nome_arquivo=f"Base SERASA Ate 999 {data_arquivo}.xlsx",
                            help_download="Arquivo Excel com autos da base SERASA cuja soma não decadente por CPF/CNPJ é até R$ 999,99.",
                            producer=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, filtro_valor='ate_999'),
                            empty_warning="⚠️ Nenhum auto da base SERASA encontrado até R$ 999,99.",
                            success_template="✅ {qtd:,} autos não decadentes da base SERASA até R$ 999,99",
                            producer_decadentes=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, filtro_valor='ate_999', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes Base SERASA ≤ R$ 999,99",
                            label_download_decadentes="📥 Download Decadentes Base SERASA ≤ R$ 999,99",
                            nome_arquivo_decadentes=f"Base SERASA Ate 999 Decadentes {data_arquivo}.xlsx"
                        )

                    with col_apenas_2:
                        st.markdown("##### 📥 Autos R$ 500–R$ 999,99")
                        render_exportacao_excel(
                            chave="base_serasa_500_999",
                            titulo_gerar="Gerar arquivo Base SERASA R$ 500–R$ 999,99",
                            label_download="📥 Download Base SERASA R$ 500–R$ 999,99",
                            nome_aba="Base_SERASA_500_999",
                            nome_arquivo=f"Base SERASA 500 a 999 {data_arquivo}.xlsx",
                            help_download="Arquivo Excel com autos da base SERASA cuja soma não decadente por CPF/CNPJ está entre R$ 500,00 e R$ 999,99.",
                            producer=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, filtro_valor='entre_500_999'),
                            empty_warning="⚠️ Nenhum auto da base SERASA encontrado na faixa R$ 500–R$ 999,99.",
                            success_template="✅ {qtd:,} autos não decadentes da base SERASA na faixa R$ 500–R$ 999,99",
                            producer_decadentes=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, filtro_valor='entre_500_999', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes Base SERASA R$ 500–R$ 999,99",
                            label_download_decadentes="📥 Download Decadentes Base SERASA R$ 500–R$ 999,99",
                            nome_arquivo_decadentes=f"Base SERASA 500 a 999 Decadentes {data_arquivo}.xlsx"
                        )

                    with col_apenas_3:
                        st.markdown("##### 📥 Autos acima de R$ 1.000,00")
                        render_exportacao_excel(
                            chave="base_serasa_acima_1000",
                            titulo_gerar="Gerar arquivo Base SERASA > R$ 1.000,00",
                            label_download="📥 Download Base SERASA > R$ 1.000,00",
                            nome_aba="Base_SERASA_Acima_1000",
                            nome_arquivo=f"Base SERASA Acima 1000 {data_arquivo}.xlsx",
                            help_download="Arquivo Excel com autos da base SERASA cuja soma não decadente por CPF/CNPJ é acima de R$ 1.000,00.",
                            producer=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, filtro_valor='acima_1000'),
                            empty_warning="⚠️ Nenhum auto da base SERASA encontrado acima de R$ 1.000,00.",
                            success_template="✅ {qtd:,} autos não decadentes da base SERASA acima de R$ 1.000,00",
                            producer_decadentes=lambda: preparar_dados_apenas_serasa(df_base_serasa_export, filtro_valor='acima_1000', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes Base SERASA > R$ 1.000,00",
                            label_download_decadentes="📥 Download Decadentes Base SERASA > R$ 1.000,00",
                            nome_arquivo_decadentes=f"Base SERASA Acima 1000 Decadentes {data_arquivo}.xlsx"
                        )
                else:
                    st.warning("⚠️ Base SERASA não disponível para exportação por faixa.")

                # Exportação apenas dos autos da SERASA que não constam na Dívida Ativa.
                st.markdown("---")
                st.markdown("#### 📥 Exportar SERASA excluindo o que está na Dívida Ativa")
                st.info("💡 Esta exportação contém somente os autos da base SERASA que **não constam na Dívida Ativa**. Mantém os mesmos filtros da Base SERASA: autuados cobraveis, deduplicação por auto, valores positivos, remoção de decadentes nos arquivos principais e botão separado para decadentes.")
                st.caption("📐 **Como é feita a conta:** Apenas SERASA = autos únicos da SERASA − autos únicos que também estão na Dívida Ativa. As faixas de valor usam a **soma dos autos não decadentes por CPF/CNPJ**.")
                if 'df_autos_apenas_serasa' in resultados and not resultados['df_autos_apenas_serasa'].empty:
                    df_apenas_serasa_export = resultados['df_autos_apenas_serasa']
                    st.markdown("##### 📥 Todos os Autos SERASA sem Dívida")
                    st.markdown("**Sem filtro de valor**")
                    render_exportacao_excel(
                        chave="serasa_sem_divida_todos",
                        titulo_gerar="Gerar arquivo SERASA sem Dívida",
                        label_download="📥 Download SERASA sem Dívida",
                        nome_aba="SERASA_Sem_Divida",
                        nome_arquivo=f"Base SERASA Sem Divida {data_arquivo}.xlsx",
                        help_download="Arquivo Excel com autos que estão na SERASA e não constam na Dívida Ativa.",
                        producer=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export),
                        empty_warning="⚠️ Nenhum dado disponível para exportar em SERASA sem Dívida.",
                        success_template="✅ {qtd:,} autos não decadentes na SERASA sem Dívida",
                        producer_decadentes=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes - SERASA sem Dívida",
                        label_download_decadentes="📥 Download Decadentes - SERASA sem Dívida",
                        nome_arquivo_decadentes=f"Base SERASA Sem Divida Decadentes {data_arquivo}.xlsx"
                    )

                    col_sem_divida_1, col_sem_divida_2, col_sem_divida_3 = st.columns(3)

                    with col_sem_divida_1:
                        st.markdown("##### 📥 Autos até R$ 999,99")
                        render_exportacao_excel(
                            chave="serasa_sem_divida_ate_999",
                            titulo_gerar="Gerar arquivo SERASA sem Dívida ≤ R$ 999,99",
                            label_download="📥 Download SERASA sem Dívida ≤ R$ 999,99",
                            nome_aba="SERASA_Sem_Divida_Ate_999",
                            nome_arquivo=f"Base SERASA Sem Divida Ate 999 {data_arquivo}.xlsx",
                            help_download="Arquivo Excel com autos da SERASA sem Dívida cuja soma não decadente por CPF/CNPJ é até R$ 999,99.",
                            producer=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, filtro_valor='ate_999'),
                            empty_warning="⚠️ Nenhum auto da SERASA sem Dívida encontrado até R$ 999,99.",
                            success_template="✅ {qtd:,} autos não decadentes da SERASA sem Dívida até R$ 999,99",
                            producer_decadentes=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, filtro_valor='ate_999', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes SERASA sem Dívida ≤ R$ 999,99",
                            label_download_decadentes="📥 Download Decadentes SERASA sem Dívida ≤ R$ 999,99",
                            nome_arquivo_decadentes=f"Base SERASA Sem Divida Ate 999 Decadentes {data_arquivo}.xlsx"
                        )

                    with col_sem_divida_2:
                        st.markdown("##### 📥 Autos R$ 500–R$ 999,99")
                        render_exportacao_excel(
                            chave="serasa_sem_divida_500_999",
                            titulo_gerar="Gerar arquivo SERASA sem Dívida R$ 500–R$ 999,99",
                            label_download="📥 Download SERASA sem Dívida R$ 500–R$ 999,99",
                            nome_aba="SERASA_Sem_Divida_500_999",
                            nome_arquivo=f"Base SERASA Sem Divida 500 a 999 {data_arquivo}.xlsx",
                            help_download="Arquivo Excel com autos da SERASA sem Dívida cuja soma não decadente por CPF/CNPJ está entre R$ 500,00 e R$ 999,99.",
                            producer=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, filtro_valor='entre_500_999'),
                            empty_warning="⚠️ Nenhum auto da SERASA sem Dívida encontrado na faixa R$ 500–R$ 999,99.",
                            success_template="✅ {qtd:,} autos não decadentes da SERASA sem Dívida na faixa R$ 500–R$ 999,99",
                            producer_decadentes=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, filtro_valor='entre_500_999', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes SERASA sem Dívida R$ 500–R$ 999,99",
                            label_download_decadentes="📥 Download Decadentes SERASA sem Dívida R$ 500–R$ 999,99",
                            nome_arquivo_decadentes=f"Base SERASA Sem Divida 500 a 999 Decadentes {data_arquivo}.xlsx"
                        )

                    with col_sem_divida_3:
                        st.markdown("##### 📥 Autos acima de R$ 1.000,00")
                        render_exportacao_excel(
                            chave="serasa_sem_divida_acima_1000",
                            titulo_gerar="Gerar arquivo SERASA sem Dívida > R$ 1.000,00",
                            label_download="📥 Download SERASA sem Dívida > R$ 1.000,00",
                            nome_aba="SERASA_Sem_Divida_Acima_1000",
                            nome_arquivo=f"Base SERASA Sem Divida Acima 1000 {data_arquivo}.xlsx",
                            help_download="Arquivo Excel com autos da SERASA sem Dívida cuja soma não decadente por CPF/CNPJ é acima de R$ 1.000,00.",
                            producer=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, filtro_valor='acima_1000'),
                            empty_warning="⚠️ Nenhum auto da SERASA sem Dívida encontrado acima de R$ 1.000,00.",
                            success_template="✅ {qtd:,} autos não decadentes da SERASA sem Dívida acima de R$ 1.000,00",
                            producer_decadentes=lambda: preparar_dados_apenas_serasa(df_apenas_serasa_export, filtro_valor='acima_1000', somente_decadentes=True),
                            titulo_gerar_decadentes="Gerar arquivo Decadentes SERASA sem Dívida > R$ 1.000,00",
                            label_download_decadentes="📥 Download Decadentes SERASA sem Dívida > R$ 1.000,00",
                            nome_arquivo_decadentes=f"Base SERASA Sem Divida Acima 1000 Decadentes {data_arquivo}.xlsx"
                        )
                else:
                    st.warning("⚠️ Nenhum auto encontrado na SERASA fora da Dívida Ativa.")

                # Exportação da base inteira já classificada.
                st.markdown("---")
                st.markdown("#### 📥 Exportar base SERASA com classificação de autuado")
                st.info("💡 Exporta a base SERASA com classificação de autuado. O arquivo principal traz os registros não decadentes; o botão separado baixa somente os decadentes. Também adiciona **Classificação Autuado**, **Motivo Classificação** e **Termo Identificado**.")
                if 'df_serasa_original' in resultados and resultados['df_serasa_original'] is not None and not resultados['df_serasa_original'].empty:
                    render_exportacao_excel(
                        ...
                        success_template="✅ {qtd:,} registros não decadentes com classificação",
                        extra_caption_fn=get_resumo_classificacao_autuado,
                        producer_decadentes=lambda: preparar_dados_serasa_classificacao(resultados['df_serasa_original'], somente_decadentes=True),
                        titulo_gerar_decadentes="Gerar arquivo Decadentes - Base SERASA com classificação",
                        label_download_decadentes="📥 Download Decadentes - Base SERASA com classificação",
                        nome_arquivo_decadentes=f"Base SERASA Classificacao Autuados Decadentes {data_arquivo}.xlsx"
                    )
                else:
                ...
                st.caption(f"💡 **Classificação por faixa de valor:** Baseada na SOMA dos valores não decadentes por CPF/CNPJ. Cada arquivo principal exclui autos decadentes, e os decadentes ficam em download separado.")
    ...
    with tab3:
        ...
        st.info("💡 Baixe as planilhas separadas por valor em formato Excel (.xlsx). Os arquivos principais excluem autos decadentes; cada faixa também tem download separado só de decadentes.")
        ...
        with col_exp1:
            ...
            if not resultados['serasa_abaixo_1000_ind'].empty:
                render_exportacao_excel(
                    ...
                    success_template="✅ {qtd:,} autos de infração não decadentes",
                    producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor='ate_999', somente_decadentes=True),
                    titulo_gerar_decadentes="Gerar planilha Decadentes ≤ R$ 999,99",
                    label_download_decadentes="📥 Download Decadentes ≤ R$ 999,99 (Excel)",
                    nome_arquivo_decadentes=f"Autos Ate 999 Decadentes {datetime.now().strftime('%d %m %Y %H:%M')}.xlsx"
                )
            else:
        ...
        with col_exp2:
            ...
            if not resultados['serasa_500_999_acum_autos'].empty:
                render_exportacao_excel(
                    ...
                    success_template="✅ {qtd:,} autos de infração não decadentes",
                    producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor='entre_500_999', somente_decadentes=True),
                    titulo_gerar_decadentes="Gerar planilha Decadentes R$ 500–R$ 999,99",
                    label_download_decadentes="📥 Download Decadentes R$ 500–R$ 999,99 (Excel)",
                    nome_arquivo_decadentes=f"Autos 500 a 999 Decadentes {datetime.now().strftime('%d %m %Y %H:%M')}.xlsx"
                )
            else:
                st.warning("Nenhum auto encontrado nesta faixa de valor")
        ...
        with col_exp3:
            ...
            if not resultados['serasa_acima_1000_ind'].empty:
                render_exportacao_excel(
                    ...
                    success_template="✅ {qtd:,} autos de infração não decadentes",
                    producer_decadentes=lambda: preparar_dados_exportacao(df_export, filtro_valor='acima_1000', somente_decadentes=True),
                    titulo_gerar_decadentes="Gerar planilha Decadentes > R$ 1.000,00",
                    label_download_decadentes="📥 Download Decadentes > R$ 1.000,00 (Excel)",
                    nome_arquivo_decadentes=f"Autos Acima 1000 Decadentes {datetime.now().strftime('%d %m %Y %H:%M')}.xlsx"
                )
            else:
                st.warning("Nenhum auto encontrado nesta faixa de valor")
